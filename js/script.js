...
firebase.auth().onAuthStateChanged((user) => {
    if (user) {
        ...
        dbRef = database.ref('tabela-geral');
    ...
    // Escuta quando uma linha que JÁ EXISTE for alterada
    dbRef.on('child_changed', (snapshot) => {
        const key = snapshot.key;          // A chave da linha alterada
        const dadosAlterados = snapshot.val(); // Os novos dados da linha (objeto completo)

        // 1. Localizar a linha na tabela HTML usando a data-key
        const linhaParaAtualizar = document.querySelector(`tr[data-key="${key}"]`);

        if (linhaParaAtualizar) {
            // 2. Atualizar as células desta linha com os novos valores
            // Fazemos um loop pelos campos ou atualizamos um a um
            Object.keys(dadosAlterados).forEach(campo => {
                const celula = linhaParaAtualizar.querySelector(`td[data-field="${campo}"]`);
                if (celula) {
                    celula.innerText = dadosAlterados[campo];
                    
                    // Opcional: Feedback visual de que o campo mudou (piscar amarelo)
                    celula.style.backgroundColor = '#fff9c4'; 
                    setTimeout(() => celula.style.backgroundColor = '', 500);
                }
            });
        }
    });
    ...
    const filtroDebounced = debounce(aplicarFiltros, 400);//atrasa o filtro em 600ms, para a função não ser chamada assim que uma nova tecla é digitada
    ...
    refTabelas.addEventListener('click', function(event) {            
        ...
        if (event.target.classList.contains('btn-slr')) {
            ...
            if(corBotao == 'rgb(0, 120, 0)'){
                ...
                idClassificacao = linha.id
                
                // Extrair Nome cargo e vocativo
                const nome = celulas[2].textContent;
                const cargo = celulas[3].textContent;
                const vocativo = celulas[4].textContent;
                    
                // --- NOVA LÓGICA: ADICIONAR AO FIREBASE ---
                const novoDado = {
                    nome: nome.trim(),
                    cargo: cargo.trim(),
                    vocativo: vocativo.trim(),
    ...
    function alterarTexto(e) {
        ...
        const celulaClicada = e.target.closest('td');

        if (!key || !celulaClicada) return;

        // 1. Descobrir qual campo do Firebase vamos alterar
        const campo = celulaClicada.getAttribute('data-field');
        if (!campo) return;

        // 2. Pegar o valor atual e solicitar o novo
        const valorAtual = celulaClicada.innerText;
        const novoValor = prompt(`Editar ${campo}:`, valorAtual);

        // 3. Validar se o usuário não cancelou ou deixou vazio
        if (novoValor !== null && novoValor !== valorAtual) {
            atualizarNoFirebase(key, campo, novoValor, celulaClicada);
        }
    }
    function atualizarNoFirebase(key, campo, novoValor, elementoHtml) {
        // 4. Referenciar o caminho exato no Firebase
        // Exemplo: 'usuarios/' + key
        const updates = {};
        updates[campo] = novoValor;

        firebase.database().ref('tabela-geral/' + key).update(updates)
            .then(() => {
                // 5. Atualizar a interface apenas se o Firebase confirmou
    ...
    //FUNÇÃO PARA ADICIONAR NOVA LINHA NA TABELA E NO BANCO DE DADOS
    async function salvarDados() {
        // 1. Captura os elementos de input
        ...
        try {            
            ...
            // 8. Fecha o modal automaticamente
            fecharModal();
        }
        catch(error) {
            console.error("Erro ao salvar no Firebase:", error);
    ...
    function adicionarLinhaSalva(snapshot) {
        ...
        if (data.idClassificacao !== "") {
            novaLinha.id = data.idClassificacao
            novaLinha.dataset.tabela = data.tabelaEscolhida
        };
        ...
        tdNome.dataset.field = 'nome';//para definir a célula que foi clicada para alteração
        ...
        tdCargo.dataset.field = 'cargo';

        const tdVocativo = document.createElement("td");
        tdVocativo.textContent = data.vocativo;
                
        //Adicionar as células à linha
        novaLinha.appendChild(tdVocativo);
    ...
    function povoarTbRelatorios() {
        ...
        cabecalho.id = "cabecalho-tabela-relatorio"
        ...
        linhasSalvos.forEach(linha => {
            ...
            // 2. LOGICA DE FILTRO:
            // Supondo que 'filtroGlobal' seja sua variável com o valor desejado
            const valorFiltro = minhaTabelaAtual; // Substitua pelo nome da sua variável global
            const tabelaDaLinha = linha.dataset.tabela;

            // Se o filtro estiver ativo e for diferente da tabela da linha, ignora esta linha
            if (valorFiltro && valorFiltro !== "" && tabelaDaLinha !== valorFiltro) {
                return; // Pula para a próxima iteração do forEach (não cria a linha)
            }
            
            const tdLinha = document.createElement('tr');
            tdLinha.dataset.tabela = tabelaDaLinha
            const tdNomeCargo = document.createElement('td');
            tdNomeCargo.style.textAlign = 'center';            
            const classificacao = parseInt(linha.id)            
            tdNomeCargo.setAttribute('data-sort', classificacao);          

            
            //O bloco abaixo define a ordem de montagem da tabela relatório
            for (let i = 0; i < linha.cells.length; i++) {                
                ...
                if (i === 0) {
                    // Guarda o que deve ir para o topo
                    nome = `${texto}<br>`;
                    } else if (i === 2) {                   
                    vocativo = `${texto}<br>`;
                    } else {
                    // Acumula os demais campos
                    cargo = `<strong>${texto}</strong><br>`;
                }
                tdNomeCargo.innerHTML += vocativo + nome + cargo;
            }
    ...
    //Capturar a linha da tabela salvos em que foi clicada:
    const meuElemento = document.getElementById('tabela-salvos');
    meuElemento.addEventListener('dblclick', alterarTexto);
...
const botao = document.getElementById('btn-descer');
botao.addEventListener('click', () => {
  window.scrollBy({
    top: document.body.scrollHeight, // Quantidade de pixels para descer
    behavior: 'smooth' // Faz a rolagem ser suave, não um pulo seco
  });
});


//INSERIR LINHA NA TABELA, TODAS AS FUNÇÕEA ABAIXO:
// 1. Função para abrir o formulário (esta deve ser chamada pelo seu botão principal)
function abrirFormulario() {
    const modal = document.getElementById('modalFormulario');
    
    if (modal) {
        // Forçamos o estilo via atributo 'style' para garantir prioridade total
        modal.style.cssText = "display: flex !important; position: fixed !important; z-index: 999999 !important; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(0,0,0,0.8); justify-content: center; align-items: center;";
    } else {
        alert("Erro crítico: Elemento modalFormulario não encontrado no HTML!");
    }
}
...
//AS FUNÇÕEA BAIXO SÃO PARA ESCOLHER E FECHAR O MODAL DAS OPÇÕES DE TABELA:
// 1. Função apenas para ABRIR o modal das opções de tabela
function abrirOpcoesTabela() {
    const modal = document.getElementById('modalOpcaoTabela');
    if (modal) {
        modal.style.display = 'flex';
        modal.style.cssText = "display: flex !important; position: fixed !important; z-index: 999999 !important; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(0,0,0,0.8); justify-content: center; align-items: center;";
    }
}
// 2. Função que roda AUTOMATICAMENTE ao escolher uma opção
function finalizarEscolha() {
    const select = document.getElementById('selectTabela');
    const modal = document.getElementById('modalOpcaoTabela');
    
    // Atualiza a variável global com o novo valor
    minhaTabelaAtual = select.value;

    /* 2. RECRIA a referência do Firebase para o novo caminho
    dbRef = firebase.database().ref(minhaTabelaAtual);*/
        
    // Fecha o modal automaticamente
    if (modal) {
        modal.style.display = 'none';
    }    
    alert("Tabela de destino alterada para: " + minhaTabelaAtual);
}
...
function abrirModalConsulta() {
    const modal = document.getElementById('modalConsultaPreced');
    if (modal) {
        modal.style.display = 'flex';
        modal.style.cssText = "display: flex !important; position: fixed !important; z-index: 999999 !important; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(0,0,0,0.8); justify-content: center; align-items: center;";
    }
...
//Função para renderizar a tabela de presentes no modal para consulta:
function renderizarCargosNoModal() {
    /*
    // Exemplo de como chamar para uma tabela específica: renderizarCargosNoModal('tabela-2')
    function renderizarCargosNoModal(nomeTabela = null) {
        const seletor = nomeTabela ? `tr[data-tabela="${nomeTabela}"]` : 'tr[data-tabela]';
        const linhas = document.querySelectorAll(seletor);
        // ... resto do código igual
    }
    */
    const corpoModal = document.getElementById('corpoTabelaModal');
    
    // Limpa o conteúdo anterior
    corpoModal.innerHTML = '';

    // O seletor [data-tabela] pega qualquer elemento que tenha esse atributo
    const linhas = document.querySelectorAll('tr[data-tabela]');

    linhas.forEach(linha => {
        const id = linha.getAttribute('id');
        // Usamos querySelector para achar o cargo dentro desta linha específica
        const celulaCargo = linha.querySelector('[data-field="cargo"]');

        if (celulaCargo) {
            const cargo = celulaCargo.innerText;

            // Cria a nova linha para o modal
            const tr = document.createElement('tr');
            tr.style.borderBottom = "1px solid #eee";
            
            tr.innerHTML = `
                <td style="padding: 10px; white-space: nowrap; font-weight: bold; text-align: center; width: 1%;">
                    ${id}
                </td>
            ...
            corpoModal.appendChild(tr);
        }
    });
    // Inicializa o Tablesort após a estrutura estar correta e no DOM
        try {
            new Tablesort(tabelaPrecedenciaModal);
        } catch (e) {
            console.error("Erro ao carregar Tablesort:", e);
        }

    // Se usar o Tablesorter, lembre de dar o update aqui:
    // $( "#tabelaPrecedenciaModal" ).trigger( "update" ); 
}
